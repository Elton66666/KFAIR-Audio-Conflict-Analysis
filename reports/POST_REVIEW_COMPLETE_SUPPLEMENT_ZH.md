...
| 91.67% 单次高分可能偶然 | 主结果改为六折均值±折间 SD，并报告逐折和 Wilcoxon | 基线 43.66%±9.21%；修复 75.52%–77.95%；六折方向一致，p=0.03125 | 91.67%及相近数字只能描述特定 actor 21–24 划分，不得写成普遍性能 |
| LoRA 是强基线，定位是否必要 | Kimi 六折等参数 LoRA；Qwen 等参数 full adapter/LoRA；真实扰动比较 | 因果定位的价值是解释故障并提供可控干预位置，不是宣称 adapter 永远优于 LoRA | LoRA 在部分指标和扰动条件上最佳；论文必须正面承认 |
...
| 构造/合成冲突 | 时间外全量 EMIS、真实声学扰动加强了边界验证 | 可更稳健地讨论时间外与声学退化，不再只依赖单一小子集 | 尚未加入 CASE 等真实场景冲突；应列为 limitation，而非临时扩展新故事 |
...
三种修复相对基线平均提升 +31.86、+32.47、+34.29 pp；对六个折的双侧 Wilcoxon 均为 p=0.03125。论文摘要、主表和结论应使用这些六折数字。actor 21–24 上 91.32%、86.28%、93.58%等结果只能作为逐折结果，不得替代六折总体。

### 4.2 Qwen 第二模型应拆成三个层次表述
...
- 主修复表必须从单一 actor 21–24 结果改为六折均值±SD；逐折结果移到补充材料。
- Qwen 只需一行或一个紧凑小表：baseline、decision adapter、同参数 full adapter、同参数 LoRA；注明测试仍为 32 条。
- 外部/保持表应优先放：EMIS 全量 conflict acoustic follow、最强组合扰动、RAVDESS/TESS WER。六种扰动完整表和逐种子 p 值放补充材料或仓库。

### 5.3 因果定位与 LoRA 的关系

论文不能把故事写成“只有因果定位才能获得性能”。实验显示普通 LoRA 是非常强的工程基线，在 Kimi 六折均值、部分真实扰动和匹配样本保持上可达到最高。更严谨的分工是：
...
下面完整收录三份技术报告。它们按实验发生顺序保留，因此较早报告中的“尚未完成”“只能方向性表述”等状态，可能已被后续工作更新。论文写作时遵循以下优先级：
...
在修改已有论文时，请按以下顺序执行，而不是简单把所有数字堆入正文。

1. 用六折 Kimi 结果替换摘要、主表和结论中的单折 91.67%叙述；保留逐折结果到补充材料。
2. 保留诊断→因果定位→修复的主轴；Qwen、真实扰动、ASR 和 EMIS 都是验证环节，不另立新故事。
3. 明确 Qwen 是跨架构行为复现，不是 Kimi 原生双流和相同内部层的复制。
4. 将 57,344 参数写成三类公平比较的共同预算；不要写 adapter 全面优于 LoRA。
5. 用 EMIS 全量 1,248 条替代 Kimi 192 子集作为最终外部结果；同时注明 Qwen EMIS 仍为 192 子集。
6. 用“modest but consistent and paired-significant external improvement”描述 Kimi 全量 EMIS，保留合成数据限制。
7. 用最困难组合扰动一列证明收益未在真实噪声/RIR/电话信道下消失；不要把 baseline 加噪偶尔上升解释成噪声有益。
8. 用扩大 ASR 证明修复没有破坏文本内容；TESS 参考是完整载句 `Say the word <target>`。
9. 在 limitations 中主动写明：真实录制冲突缺失、Qwen 测试小、层选择非 nested、LoRA 强、真实扰动基础集小。
10. 所有论文数字回查 GitHub `results/statistics/` 与 `results/raw_predictions/`；不要只从本报告手抄后失去可追溯性。
