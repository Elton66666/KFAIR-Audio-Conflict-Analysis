...
OUT = ROOT / "outputs"
pred = pd.DataFrame(json.loads(x) for x in (OUT / "tess_factorial_predictions.jsonl").read_text(encoding="utf-8").splitlines())
eff = pd.read_csv(OUT / "tess_factorial_effects.csv")
...
stats.to_csv(OUT / "tess_factorial_statistics.csv", index=False)
...
pair_stats.to_csv(OUT / "tess_by_emotion_pair.csv", index=False)
