...
PRED_PATH = OUT_DIR / "ravdess_choice_predictions.jsonl"
SUMMARY_PATH = OUT_DIR / "ravdess_choice_summary.json"
MANIFEST_PATH = OUT_DIR / "ravdess_eval_manifest.csv"
...
def prepare_manifest():
    ...
    for _, row in df.iterrows():
        ...
        if int(actor) <= 5 and statement == "01" and repetition == "01" and intensity == "01" and row["style"] in LABELS:
    ...
    assert len(manifest) == 20, f"expected 20 rows, got {len(manifest)}"
    assert manifest.groupby("label").size().to_dict() == {x: 5 for x in LABELS}
...
def parse_label(text):
    ...
    choice_map = {"a": "neutral", "b": "happy", "c": "sad", "d": "angry"}
...
def summarize():
    ...
    pd.DataFrame([{"mode": k, **{x: y for x, y in v.items() if x != "confusion_matrix"}} for k, v in summary.items()]).to_csv(OUT_DIR / "ravdess_choice_summary.csv", index=False)
    return summary
