...
PRED_PATH = OUT_DIR / "ravdess_statement2_emotion_predictions.jsonl"
SUMMARY_PATH = OUT_DIR / "ravdess_statement2_emotion_summary.json"
MANIFEST_PATH = OUT_DIR / "ravdess_statement2_manifest.csv"
...
def prepare_manifest():
    ...
    for _, row in df.iterrows():
        ...
        if int(actor) <= 24 and statement == "02" and repetition == "01" and intensity == "01" and row["style"] in LABELS:
...
def summarize():
    ...
    pd.DataFrame([{"mode": k, **{x: y for x, y in v.items() if x != "confusion_matrix"}} for k, v in summary.items()]).to_csv(OUT_DIR / "ravdess_statement2_emotion_summary.csv", index=False)
    return summary
