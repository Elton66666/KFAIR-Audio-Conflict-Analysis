...
PRED_PATH = OUT_DIR / "ravdess_branch_predictions.jsonl"
SUMMARY_PATH = OUT_DIR / "ravdess_branch_summary.json"
...
def summarize():
    ...
    pd.DataFrame([{"mode": k, **{x: y for x, y in v.items() if x != "confusion_matrix"}} for k, v in summary.items()]).to_csv(OUT_DIR / "ravdess_branch_summary.csv", index=False)
    return summary
